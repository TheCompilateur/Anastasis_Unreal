...
(`DefaultTreeMeshPath` dans `AnastasisPresentationRegistry.cpp` — le point de branchement
a migré depuis `AnastasisPresentation::TreeMeshPath` lors de l'intégration au tronc, la
présentation étant devenue data-driven ; le fallback code est ce qui reste à câbler en dur). Ruin serait un candidat tout aussi valable et
